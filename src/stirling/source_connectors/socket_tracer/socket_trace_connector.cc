...
void SocketTraceConnector::TransferDataImpl(ConnectorContext* ctx) {
  ...
  for (const auto& conn_tracker : conn_trackers_mgr_.active_trackers()) {
    ...
    if (transfer_spec.transfer_fn != nullptr) {
      ...
      // std::monostate.
      ECHECK((
          conn_tracker->send_data().Empty<protocols::http::stream_id, protocols::http::Message>()));
      ECHECK((
          conn_tracker->recv_data().Empty<protocols::http::stream_id, protocols::http::Message>()));
    }
