...
TEST(CassStitcherTest, OutOfOrderMatchingWithMissingResponses) {
  ...
  LOG(INFO) << "req0_s0_frame: " << req0_s0_frame.hdr.stream << " " << req0_s0_frame.timestamp_ns;
  ...
  LOG(INFO) << "2. Printing req_frames";
  LOG(INFO) << "------------------";
  for (auto& frame : req_frames) {
    LOG(INFO) << frame.hdr.stream << " " << frame.timestamp_ns;
  }
  LOG(INFO) << "2. Printing resp_frames";
  LOG(INFO) << "------------------";
  for (auto& frame : resp_frames) {
    LOG(INFO) << frame.hdr.stream << " " << frame.timestamp_ns;
  }
  ...
  LOG(INFO) << "3. Printing req_frames";
  LOG(INFO) << "------------------";
  for (auto& frame : req_frames) {
    LOG(INFO) << frame.hdr.stream << " " << frame.timestamp_ns;
  }
  LOG(INFO) << "3. Printing resp_frames";
  LOG(INFO) << "------------------";
  for (auto& frame : resp_frames) {
    LOG(INFO) << frame.hdr.stream << " " << frame.timestamp_ns;
  }
