...
RecordsWithErrorCount<Record> StitchFrames(std::deque<Frame>* req_frames,
  ...
  for (auto& resp_frame : *resp_frames) {
    ...
    while (it != req_frames->end()) {
      ...
      // LOG(INFO) << "STREAM " << req_frame.hdr.stream << " RESP " << resp_frame.timestamp_ns;
      // for (auto& ts : stream_vec) {
      //   LOG(INFO) << "\t" << ts;
      // }
      auto stream_it =
          std::upper_bound(stream_vec.begin(), stream_vec.end(), resp_frame.timestamp_ns) - 1;
      
      // LOG(INFO) << "STREAM " << req_frame.hdr.stream << "\n\tREQ START=" << *stream_vec.begin() << "\n\tRESP TS=" << resp_frame.timestamp_ns << "\n\tRETURNED REQ TS=" << *stream_it << "\n\tstream_it + 1 is " << *(std::upper_bound(stream_vec.begin(), stream_vec.end(), resp_frame.timestamp_ns));
      ...
      if (resp_frame.hdr.stream == req_frame.hdr.stream && *stream_it == req_frame.timestamp_ns) {
        // LOG(INFO) << "STREAM " << req_frame.hdr.stream << " RESP TS=" << resp_frame.timestamp_ns << " MATCH REQ TS=" << *stream_it;
        ...
        LOG(INFO) << "Matched request and response. Stream = " << req_frame.hdr.stream
                  << " Request TS = " << req_frame.timestamp_ns
                  << " Response TS = " << resp_frame.timestamp_ns;
        found_match = true;
        req_frame.consumed = true;
        break;
      } 
      // else {
        // LOG(INFO) << "STREAM " << req_frame.hdr.stream << " RESP TS=" << resp_frame.timestamp_ns << " DOES NOT MATCH REQ TS=" << *stream_it;
      // }
      it++;
    }
  ...
  // for (auto& stream: latest_resp_by_stream) {
  //   LOG(INFO) << "STREAM " << stream.first << " LATEST RESP=" << stream.second;
  // }
  auto it = req_frames->begin();
  ...
  while (it != req_frames->end()) {
    ...
    if (frame.consumed) {
      ...
      LOG(INFO) << "Found request that will never match. Stream = " << frame.hdr.stream
                << " Request TS = " << frame.timestamp_ns;
      delete_pos = it;
      break;
    }
    it++;
  ...
  if (it != req_frames->end()) {
    ...
    LOG(INFO) << "No more requests to match.";
