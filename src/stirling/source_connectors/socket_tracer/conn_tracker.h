...
class ConnTracker : NotCopyMoveable {
 public:
  ...
  std::map<TKey, std::deque<TFrameType>>& req_frames() {
  ...
  template <typename TKey, typename TFrameType>
  const std::map<TKey, std::deque<TFrameType>>& send_frames() const {
   ...
   */
  template <typename TKey, typename TFrameType>
  std::map<TKey, std::deque<TFrameType>>& resp_frames() {
    return resp_data()->Frames<TKey, TFrameType>();
  }
  template <typename TKey, typename TFrameType>
  const std::map<TKey, std::deque<TFrameType>>& recv_frames() const {
