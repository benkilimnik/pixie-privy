...
class ConnTracker : NotCopyMoveable {
 public:
  ...
  TStateType* protocol_state() {
    ...
    if constexpr (std::is_same_v<TStateType, protocols::NoState>) {
      ...
      //   ptr->global.lazy_parsing_enabled = lazy_parsing_enabled;
      // }
      return ptr;
    }
  }
 ...
 private:
  ...
  void DataStreamsToFrames() {
    auto state_ptr = protocol_state<TStateType>();
    // Set lazy parsing to true if the protocol supports it and the feature flag is enabled.
    // LazyParsingEnabled() checks both of these conditions.
    // if (protocol() == traffic_protocol_t::kProtocolHTTP) {
    //   bool lazy_parsing_enabled = LazyParsingEnabled(protocol());
    //   if (state_ptr != nullptr) {
    //     // Protocol must implement lazy parsing and have a flag its state.
    //     if (state_ptr->global.lazy_parsing_enabled != lazy_parsing_enabled) {
    //       state_ptr->global.lazy_parsing_enabled = lazy_parsing_enabled;
    //     }
    //   }
    // }
