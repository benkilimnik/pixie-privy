...
TEST_F(DataStreamTest, LostEvent) {
  ...
  stream.ProcessBytesToFrames<http::stream_id, http::Message>(message_type_t::kRequest, &state);
  auto frames = stream.Frames<http::stream_id, http::Message>();
  ...
  stream.ProcessBytesToFrames<http::stream_id, http::Message>(message_type_t::kRequest, &state);
  frames = stream.Frames<http::stream_id, http::Message>();
...
TEST_F(DataStreamTest, StuckTemporarily) {
  ...
  stream.ProcessBytesToFrames<http::stream_id, http::Message>(message_type_t::kRequest, &state);
  const auto& requests = stream.Frames<http::stream_id, http::Message>()[0];
...
TEST_F(DataStreamTest, LateArrivalPlusMissingEvents) {
  ...
  auto empty = stream.Empty<http::stream_id, http::Message>();
  EXPECT_TRUE(empty);
...
TEST_F(DataStreamTest, CannotSwitchType) {
  ...
  stream.ProcessBytesToFrames<http::stream_id, http::Message>(message_type_t::kRequest,
                                                              &http_state);

#if DCHECK_IS_ON()
  protocols::mysql::StateWrapper mysql_state{};
  EXPECT_DEATH((stream.ProcessBytesToFrames<mysql::connection_id, mysql::Packet>(
                   message_type_t::kRequest, &mysql_state)),
               "ConnTracker cannot change the type it holds during runtime");
#else
  protocols::mysql::StateWrapper mysql_state{};
  EXPECT_THROW(stream.ProcessBytesToFrames<mysql::connection_id, mysql::Packet>(
                   message_type_t::kRequest, &mysql_state),
               std::exception);
#endif
}
...
TEST_F(DataStreamTest, SpikeCapacityWithLargeDataChunk) {
  ...
  protocols::http::StateWrapper state{};
  stream.ProcessBytesToFrames<http::stream_id, http::Message>(message_type_t::kResponse, &state);
