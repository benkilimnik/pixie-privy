...
class DataStream : NotCopyMoveable {
 public:
  ...
  void InitFrames() {
    bool check_condition = std::holds_alternative<std::monostate>(frames_) ||
                           std::holds_alternative<std::map<TKey, std::deque<TFrameType>>>(frames_);
    ...
    if (std::holds_alternative<std::monostate>(frames_)) {
      ...
      frames_ = std::map<TKey, std::deque<TFrameType>>();
   ...
   */
  template <typename TKey, typename TFrameType>
  std::map<TKey, std::deque<TFrameType>>& Frames() {
    ...
    return std::get<std::map<TKey, std::deque<TFrameType>>>(frames_);
  }

  template <typename TKey, typename TFrameType>
  const std::map<TKey, std::deque<TFrameType>>& Frames() const {
    DCHECK((std::holds_alternative<std::map<TKey, std::deque<TFrameType>>>(frames_)))
        << absl::Substitute(
  ...
  bool Empty() const {
    ...
    for (const auto& [_, frames] : std::get<std::map<TKey, std::deque<TFrameType>>>(frames_)) {
      if (!frames.empty()) {
 ...
 private:
  ...
  static void EraseExpiredFrames(
      std::chrono::time_point<std::chrono::steady_clock> expiry_timestamp,
      std::map<TKey, std::deque<TFrameType>>* frames) {
    LOG(INFO) << "Expiry timestamp: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(
                     expiry_timestamp.time_since_epoch())
                     .count()
              << "ms";
    for (auto it = frames->begin(); it != frames->end(); it++) {
      ...
      LOG(INFO) << "Distance " << std::distance(deque.begin(), iter);
...
inline std::string DebugString(const DataStream& d, std::string_view prefix) {
  ...
  int frames_size;
  if (std::holds_alternative<std::map<TKey, std::deque<TFrameType>>>(d.frames_)) {
    const auto& frames_map = std::get<std::map<TKey, std::deque<TFrameType>>>(d.frames_);
