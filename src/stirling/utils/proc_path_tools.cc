...
StatusOr<std::unique_ptr<FilePathResolver>> FilePathResolver::Create(pid_t pid) {
  ...
  PX_RETURN_IF_ERROR(fp_resolver->Init());
...
Status FilePathResolver::Init() {
  ...
  PX_RETURN_IF_ERROR(proc_parser.ReadMountInfos(kRootPID, root_mount_infos_));
...
Status FilePathResolver::SetMountNamespace(pid_t pid) {
  ...
  if (inserted) {
    ...
    PX_RETURN_IF_ERROR(proc_parser.ReadMountInfos(pid, iter->second.get()));
  }
...
StatusOr<std::filesystem::path> GetSelfPath() {
  ...
  PX_ASSIGN_OR_RETURN(std::filesystem::path self_path, proc_parser.GetExePath(getpid()));
  PX_ASSIGN_OR_RETURN(std::unique_ptr<FilePathResolver> fp_resolver,
                      FilePathResolver::Create(getpid()));
  PX_ASSIGN_OR_RETURN(self_path, fp_resolver->ResolvePath(self_path));
...
StatusOr<std::filesystem::path> ProcExe(uint32_t pid, system::ProcParser* proc_parser,
  ...
  PX_RETURN_IF_ERROR(fp_resolver->SetMountNamespace(pid));
  PX_ASSIGN_OR_RETURN(const std::filesystem::path resolved_proc_exe,
